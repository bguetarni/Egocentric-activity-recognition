...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, action='store', required=True, help='path to the model to test')
    ...
    for metric_name, metric_value in metrics.items():
        if 'confusion_matrix' in metric_name:  # plot and save the confusion matrices
            ...
            heatmap.get_figure().savefig(fname='{}_{}.png'.format(model.name, metric_name))
            print('confusion matrix saved in {}_{}.png'.format(model.name, metric_name))
        else:  # display metric
